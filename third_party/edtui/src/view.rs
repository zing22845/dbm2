...
impl Widget for EditorView<'_, '_> {
    ...
    fn render(self, area: Rect, buf: &mut Buffer) {
        ...
        if wrap_lines {
            // `offset_y` is an absolute visual-row index. Skip wrap segments until
            // that row, including mid-line skips for a single long wrapped line.
            let mut visual_skip = offset_y;
            for (row_index, line) in lines.iter_row().enumerate() {
                ...
                let mut wrapped = LineWrapper::wrap_spans(spans, width, tab_width);
                if wrapped.is_empty() {
                    wrapped.push(Vec::new());
                }
                let total_segs = wrapped.len();
                if visual_skip >= total_segs {
                    visual_skip -= total_segs;
                    continue;
                }

                let start_seg = visual_skip;
                visual_skip = 0;
                let visible: Vec<_> = wrapped.into_iter().skip(start_seg).collect();
                let render_line = RenderLine::Wrapped(visible);
                let show_precedes = start_seg > 0;
                let precedes_origin = (content_area.x, content_area.y);

                if line_numbers_enabled {
                    if let Some(gutter) = gutter_row_area {
                        // Keep the line number on the first *visible* row of this
                        // logical line, even when earlier wrap segments scrolled away.
                ...
                if row_index == cursor.row {
                    // Recompute position on the full wrapped line, then subtract
                    // skipped segments so the cursor aligns with the clipped view.
                    let full_spans = generate_spans(
                        line,
                        &selections,
                        &self.state.highlights,
                        row_index,
                    ...
                    let mut full_wrapped = LineWrapper::wrap_spans(full_spans, width, tab_width);
                    if full_wrapped.is_empty() {
                        full_wrapped.push(Vec::new());
                    }
                    let full = RenderLine::Wrapped(full_wrapped);
                    let pos = full.data_coordinate_to_screen_coordinate(
                        cursor.col,
                        content_area,
                        tab_width,
                    );
                ...
                // Vim-style precedes marker: content above this wrap window is
                // scrolled away; line number stays in the gutter to the left.
                if show_precedes {
                    let style = Style::default().fg(Color::Blue);
                    for (i, ch) in ['<', '<', '<'].into_iter().enumerate() {
                        let x = precedes_origin.0.saturating_add(i as u16);
                        if x >= precedes_origin.0.saturating_add(width as u16) {
                            break;
                        }
                        if let Some(cell) = buf.cell_mut(Position::new(x, precedes_origin.1)) {
                            cell.set_char(ch);
            ...
            for (row_index, line) in (offset_y..).zip(lines.iter_row().skip(row_index)) {
                ...
                let render_line = RenderLine::Single(spans);
                ...
                if line_numbers_enabled {
                    if let Some(gutter) = gutter_row_area {
                        ...
                        gutter_row_area = Some(Rect::new(
                            gutter.x,
                            gutter.y.saturating_add(1),
                            gutter.width,
                            gutter.height.saturating_sub(1),
                        ));
                    }
                }
