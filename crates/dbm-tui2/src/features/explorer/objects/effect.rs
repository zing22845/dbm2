...
impl Effect for ObjectsEffect {
    ...
    fn run(self, _emit: Emitter<Self::Action>, services: Arc<Services>) -> BoxFuture<Vec<Self::Action>> {
        Box::pin(async move {
            match self {
                ObjectsEffect::LoadDatabases { instance, connection } => {
                ...
                ObjectsEffect::LoadSchemas { instance, connection, database } => {
                ...
                ObjectsEffect::LoadExtensions { instance, connection, database } => {
                    match services.list_extensions(&instance, &connection, &database).await {
                        Ok(extensions) => {
                            vec![ObjectsAction::ExtensionsLoaded { database, extensions }]
                        }
                        Err(error) => vec![ObjectsAction::ExtensionsError { database, error }],
                    }
                }
                ObjectsEffect::LoadObjectList { instance, connection, database, schema, kind } => {
                    let result = list_object_kind(&services, &instance, &connection, &database, &schema, kind).await;
                    match result {
...
async fn list_object_kind(
    ...
    match kind {
        ObjectKind::Tables => services.list_tables(instance, connection, database, schema).await,
        ObjectKind::Views => services.list_views(instance, connection, database, schema).await,
        ObjectKind::Matviews => {
            services.list_matviews(instance, connection, database, schema).await
        }
        ObjectKind::Procedures => {
            services.list_procedures(instance, connection, database, schema).await
        }
        ObjectKind::Functions => {
            services.list_functions(instance, connection, database, schema).await
        }
        ObjectKind::Sequences => {
            services.list_sequences(instance, connection, database, schema).await
        }
