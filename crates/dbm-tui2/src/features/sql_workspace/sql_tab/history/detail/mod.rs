...
/// display row when filtered.
pub fn scroll_on_selection_change(
    state: &mut HistoryDetailState,
    sql: &str,
    search: &PaneSearch,
    text_width_after: u16,
    ...
    if search.has_filter()
        && let Some(row) = first_match_display_line(sql, &search.query, search.options, text_width_after)
    {
        let max_scroll =
...
mod tests {
    ...
    fn scroll_on_selection_change_jumps_to_first_match() {
        let sql = "line0\nline1\nmatch here\nline3";
        let mut state = HistoryDetailState::default();
